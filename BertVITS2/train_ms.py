...
os.environ['HASTER_ADDR'] = '127.0.0.1'
os.environ['HASTER_PORT'] = '8080'
...
def run(rank, n_gpus, hps):
    ...
    dist.init_process_group(backend= PL_TORCH_DISTRIBUTED_BACKEND, init_method='env://', world_size=n_gpus, rank=rank)
    ...
    if not optim_g.param_groups[0].get("initial_lr"):
            optim_g.param_groups[0]["initial_lr"] = 0.0003
    if not optim_d.param_groups[0].get("initial_lr"):
            optim_d.param_groups[0]["initial_lr"] = 0.0003
    if not optim_dur_disc.param_groups[0].get("initial_lr"):
            optim_dur_disc.param_groups[0]["initial_lr"] = 0.0003
    # ..       
